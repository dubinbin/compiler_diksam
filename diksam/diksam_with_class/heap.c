...
{
	if (((type->basic_type == DVM_STRING_TYPE || type->basic_type == DVM_CLASS_TYPE) && type->derive_count == 0) || (type->derive_count > 0 && type->derive[0].tag == DVM_ARRAY_DERIVE)) {
		return DVM_TRUE;
	}
	return DVM_FALSE;
}
...
static void gc_mark(DVM_ObjectRef *ref){
    ...
    if (ref->data->type == ARRAY_OBJECT && ref->data->u.array.type == OBJECT_ARRAY) {
