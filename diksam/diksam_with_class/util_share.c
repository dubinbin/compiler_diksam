...
// 在本地存储器里搜寻源代码文件，并返回搜寻结果的状态，如果搜寻成功，将文件指针存储在最后一个参数中
SearchFileStatus dvm_search_file(char *search_path, char *search_file, char *found_path, FILE **fp){
	...
	for (sp_idx = 0, dp_idx = 0;; sp_idx++) {
		if (search_path[sp_idx] == FILE_PATH_SEPARATOR || search_path[sp_idx] == '\0') {
			...
			if (dp_idx == 0 && search_path[sp_idx] == '~' && (home = getenv("HOME")) != NULL) {
...
// 根据类名和方法名创建字符串分配空间，使用该函数创建的字符串需要手动释放存储空间
