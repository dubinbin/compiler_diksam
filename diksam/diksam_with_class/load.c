...
// 
static void implement_diksam_function(DVM_VirtualMachine *dvm, int dest_idx, ExecutableEntry *ee, int src_idx){
	dvm->function[dest_idx]->u.diksam_f.executable = ee;
	dvm->function[dest_idx]->u.diksam_f.index = src_idx;
}

static void add_functions(DVM_VirtualMachine *dvm, ExecutableEntry *ee){
    int src_idx;
    int dest_idx;
...
// 根据包名和函数名寻找函数，必须两者都相同才是寻找成功
static int search_function(DVM_VirtualMachine *dvm, char *package_name, char *name){
	int i;

	for (i = 0; i < dvm->function_count; i++) {
		if (dvm_compare_package_name(dvm->function[i]->package_name, package_name) && !strcmp(dvm->function[i]->name, name)) {
			return i;
		}
	}
...
// 根据包名称和类名寻找类，只有两个都相等才能寻找成功
static int search_class(DVM_VirtualMachine *dvm, char *package_name, char *name){
	int i;

	for (i = 0; i < dvm->class_count; i++) {
		if (dvm_compare_package_name(dvm->class[i]->package_name, package_name) && !strcmp(dvm->class[i]->name, name)) {
			return i;
		}
	}
...
static ExecutableEntry* add_executable_to_dvm(DVM_VirtualMachine *dvm, DVM_Executable *executable, DVM_Boolean is_top_level){
	...
	if (dvm->executable_entry == NULL) {
		...
		// 遍历执行体入口链表，将新的执行体链表插入到链表尾部
		for (ee_pos = dvm->executable_entry; ee_pos->next; ee_pos = ee_pos->next);
	...
	// 如果执行体为顶层环境，则将虚拟机的顶层环境设为这个新的执行体入口
	if (is_top_level) {
...
void DVM_set_executable(DVM_VirtualMachine *dvm, DVM_ExecutableList *list){
	...
	for (pos = list->list; pos; pos = pos->next) {
		ee = add_executable_to_dvm(dvm, pos->executable, (pos->executable == list->top_level));
	}
}
